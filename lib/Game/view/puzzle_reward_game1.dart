...
import 'package:TajwidKidz/Game/view/tebak%20huruf/tebak_huruf_level_screen.dart';
...
class PuzzleRewardGame extends StatefulWidget {
  final String gameName; // contoh: "Tebak_Huruf"
  final int level; // contoh: 1
  final String puzzleKey; // contoh: 1
  final String puzzleImagePath; // contoh: 1
  final String puzzleName; // contoh: 1
  final String puzzleLevel; // contoh: 1
  final String puzzleDescription; // contoh: 1

  const PuzzleRewardGame({
    super.key,
  ...
  @override
  State<PuzzleRewardGame> createState() => _PuzzleRewardGameState();
}

class _PuzzleRewardGameState extends State<PuzzleRewardGame> {
  ...
  // final String puzzleKey = 'puzzle_solved_MasjidilHaram';
  // final String puzzleImagePath = 'assets/images/puzzle_modul/masjidil_haram.png';
  // final String puzzleName = 'Masjidil Haram';
  // final String puzzleLevel = 'Level 1 : Tebak Huruf';
  // final String puzzleDescription = 'Masjid terbesar di dunia dan tempat berdirinya Ka\'bah, arah kiblat seluruh umat Islam. Dibangun pertama kali oleh Nabi Ibrahim dan Nabi Ismail.';
  ...
  Future<void> _setupPuzzle() async {
    ...
    try {
      final ByteData byteData = await rootBundle.load(widget.puzzleImagePath);
  ...
  void _checkIfSolved() async {
    if (correctPiecesCount == pieces.length) {
      ...
      await prefs.setBool(widget.puzzleKey, true);
          ...
          child: Column(
            ...
            children: [
              ...
              ClipRRect(
                ...
                child: Image.asset(widget.puzzleImagePath, height: 130),
              ),
              const SizedBox(height: 15),
              Text(widget.puzzleName, style: GoogleFonts.inter(fontSize: 20, fontWeight: FontWeight.bold)),
              const SizedBox(height: 6),
              Text(widget.puzzleLevel, textAlign: TextAlign.center, style: GoogleFonts.inter(fontSize: 14, color: Colors.grey[700])),
              const SizedBox(height: 12),
              Text(widget.puzzleDescription, textAlign: TextAlign.center, style: GoogleFonts.inter(fontSize: 14)),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          ClipRRect(
            borderRadius: BorderRadius.circular(12.0),
            child: Image.asset(widget.puzzleImagePath, height: 150),
          ),
        ],
      ),
    );
  ...
  Widget _buildSolvedView() {
    return Container(
      ...
      child: ClipRRect(
        borderRadius: BorderRadius.circular(12.0),
        child: Image.asset(widget.puzzleImagePath, fit: BoxFit.cover),
      ),
    );
  }
}
